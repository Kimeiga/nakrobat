...
public class HeadMouseLook : MonoBehaviour
{
    ...
    public Vector3 tempEuler;

    public MouseRotate bodyMouseLook;
    public MouseRotate fireMouseLook;
    ...
    void Update()
    {
        ...
        if (Input.GetButton("Freelook"))
        {
            ...
            fireMouseLook.enabled = false;
        }
        else {
            canRotate = false;

            rotationX = Mathf.Lerp(rotationX, 0, 0.3f);
            rotationY = Mathf.Lerp(rotationY, 0, 0.3f);

            bodyMouseLook.enabled = true;
            fireMouseLook.enabled = true;
        }
